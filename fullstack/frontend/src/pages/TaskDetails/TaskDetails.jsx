...
const TaskDetails = () => {
    ...
    const fetchTaskDetails = async () => {
      try {
        ...
        const plantResponse = await axios.get(
          `http://18.117.255.133/api/plants/${response.data.plant_id}`,
          {
            headers: {
              Authorization: "Bearer " + token,
            },
  ...
  const handleHarvestSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("http://18.117.255.133/api/harvests", harvestData, {
        headers: {
          Authorization: "Bearer " + token,
        },
      });
  ...
  const handleImageUpload = async (formData) => {
    try {
      await axios.post(
        `http://18.117.255.133/api/harvestImage/${task.id}`,
        formData,
        {
          headers: {
            Authorization: "Bearer " + token,
