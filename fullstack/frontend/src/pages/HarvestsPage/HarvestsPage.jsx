...
const HarvestsPage = () => {
  ...
  useEffect(() => {
    const fetchHarvests = async () => {
      try {
        const response = await axios.get(`http://18.117.255.133/api/harvests?user_id=${user.id}`, {
          headers: {
        ...
        const plantResponse = await axios.get(
          `http://18.117.255.133/api/plants?ids=${plantIds.join(",")}`,
          {
            headers: {
              Authorization: "Bearer " + token,
            },
  ...
  const getImageUrl = (filename) => {
    const imageUrl = `http://18.117.255.133/static/images/${filename}`;
    return imageUrl;
  };
